...
export function GoogleWorkspaceConnect({ userId, onConnected, onSkip }: Props) {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      const result = await nango.auth('google-workspace', userId)

      // Guardar connectionId en Supabase via API route
      await fetch('/api/nango/save-connection', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ connectionId: result.connectionId, userId }),
      })
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: '#777799', fontSize: 14, lineHeight: 1.7, marginBottom: 28 }}>
          Tu agente necesita acceso a Gmail, Google Docs, Calendar, Drive y Sheets para trabajar por ti. Solo lo pedimos una vez.
        </p>
